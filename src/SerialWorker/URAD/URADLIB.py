...
def loadConfiguration(ser, mode, f0, BW, Ns, Ntar, Rmax, MTI, Mth, Alpha, distance_true, velocity_true, SNR_true,
    ...
    CRC = ((configuration[0] + configuration[1] + configuration[2] + configuration[3] + configuration[4] +
            configuration[5] + configuration[6])) & 0b11111111
    configuration[7] = CRC
        ...
        if (ser.is_open):
            ...
            if (get_distance or get_velocity or get_SNR or get_movement):
                ...
                if (len(results) == results_packetLen):
                    if (get_distance or get_velocity or get_SNR):
                        ...
                        if (get_velocity):
                            velocity[0:Ntar_temp] = struct.unpack('<%df' % Ntar_temp,
                                                                  results[NtarMax * 4:NtarMax * 4 + 4 * Ntar_temp])
                        SNR[0:Ntar_temp] = struct.unpack('<%df' % Ntar_temp,
                                                         results[2 * NtarMax * 4:2 * NtarMax * 4 + 4 * Ntar_temp])
                ...
                if (len(bufferIbytes) == total_bytes):
                    ...
                    if (mode == 3 or mode == 4):
                        for i in range(two_blocks_2):
                            I[Ns + i * 2 + 0] = (bufferIbytes[(two_blocks_1 + i) * 3 + 0] << 4) + (
                                        bufferIbytes[(two_blocks_1 + i) * 3 + 1] >> 4)
                            if (Ns + i * 2 + 1 <= 2 * Ns - 1):
                                I[Ns + i * 2 + 1] = ((bufferIbytes[(two_blocks_1 + i) * 3 + 1] & 15) << 8) + \
                                                    bufferIbytes[(two_blocks_1 + i) * 3 + 2]
                        if (mode == 4):
                            for i in range(two_blocks_3):
                                I[2 * Ns + i * 2 + 0] = (bufferIbytes[
                                                             (two_blocks_1 + two_blocks_2 + i) * 3 + 0] << 4) + (
                                                                    bufferIbytes[
                                                                        (two_blocks_1 + two_blocks_2 + i) * 3 + 1] >> 4)
                                if (2 * Ns + i * 2 + 1 <= 2 * Ns + Ns_3 - 1):
                                    I[2 * Ns + i * 2 + 1] = ((bufferIbytes[(
                                                                                       two_blocks_1 + two_blocks_2 + i) * 3 + 1] & 15) << 8) + \
                                                            bufferIbytes[(two_blocks_1 + two_blocks_2 + i) * 3 + 2]
                            for i in range(two_blocks_3):
                                I[2 * Ns + Ns_3 + i * 2 + 0] = (bufferIbytes[(
                                                                                         two_blocks_1 + two_blocks_2 + two_blocks_3 + i) * 3 + 0] << 4) + (
                                                                           bufferIbytes[(
                                                                                                    two_blocks_1 + two_blocks_2 + two_blocks_3 + i) * 3 + 1] >> 4)
                                if (2 * Ns + Ns_3 + i * 2 + 1 <= 2 * Ns + 2 * Ns_3 - 1):
                                    I[2 * Ns + Ns_3 + i * 2 + 1] = ((bufferIbytes[(
                                                                                              two_blocks_1 + two_blocks_2 + two_blocks_3 + i) * 3 + 1] & 15) << 8) + \
                                                                   bufferIbytes[(
            ...
            if (get_Q):
                ...
                if (len(bufferQbytes) == total_bytes):
                    ...
                    if (mode == 3 or mode == 4):
                        for i in range(two_blocks_2):
                            Q[Ns + i * 2 + 0] = (bufferQbytes[(two_blocks_1 + i) * 3 + 0] << 4) + (
                                        bufferQbytes[(two_blocks_1 + i) * 3 + 1] >> 4)
                            if (Ns + i * 2 + 1 <= 2 * Ns - 1):
                                Q[Ns + i * 2 + 1] = ((bufferQbytes[(two_blocks_1 + i) * 3 + 1] & 15) << 8) + \
                                                    bufferQbytes[(two_blocks_1 + i) * 3 + 2]
                        if (mode == 4):
                            for i in range(two_blocks_3):
                                Q[2 * Ns + i * 2 + 0] = (bufferQbytes[
                                                             (two_blocks_1 + two_blocks_2 + i) * 3 + 0] << 4) + (
                                                                    bufferQbytes[
                                                                        (two_blocks_1 + two_blocks_2 + i) * 3 + 1] >> 4)
                                if (2 * Ns + i * 2 + 1 <= 2 * Ns + Ns_3 - 1):
                                    Q[2 * Ns + i * 2 + 1] = ((bufferQbytes[(
                                                                                       two_blocks_1 + two_blocks_2 + i) * 3 + 1] & 15) << 8) + \
                                                            bufferQbytes[(two_blocks_1 + two_blocks_2 + i) * 3 + 2]
                            for i in range(two_blocks_3):
                                Q[2 * Ns + Ns_3 + i * 2 + 0] = (bufferQbytes[(
                                                                                         two_blocks_1 + two_blocks_2 + two_blocks_3 + i) * 3 + 0] << 4) + (
                                                                           bufferQbytes[(
                                                                                                    two_blocks_1 + two_blocks_2 + two_blocks_3 + i) * 3 + 1] >> 4)
                                if (2 * Ns + Ns_3 + i * 2 + 1 <= 2 * Ns + 2 * Ns_3 - 1):
                                    Q[2 * Ns + Ns_3 + i * 2 + 1] = ((bufferQbytes[(
                                                                                              two_blocks_1 + two_blocks_2 + two_blocks_3 + i) * 3 + 1] & 15) << 8) + \
                                                                   bufferQbytes[(
                                                                                            two_blocks_1 + two_blocks_2 + two_blocks_3 + i) * 3 + 2]
