...
args=$(getopt -uo 'yhvx:r:i:c:b:u:m:' -- $*)

[ $? != 0 ] && print_help

MODES=("1: resistive  3.2\", 2.8\", 2.4\"  (320x240)" "2: 2.2\" (no touch)  (320x240)" "3: capacitive  2.8\"  (320x240)"  "4: resistive 3.5\"  (480x320)" "5: MINI  1.54\", 1.3\"  (240x240) !! W: KRN UPD !!" "6: MINI 1.14\" (135x240) !! W: KRN UPD !!" "7: ST7789V 2.0\" (NO TOUCH)  (320x240) !! W: KRN UPD !!"  "8. UNINSTALL PITFT!!!!")
set -- $args
...
for i do
    ...
    in
        "-h")
            echo "EXAMPLE:    `basename $BASH_SOURCE`  -c1 -r1 -m2   # resistive 2.8, rotation=90, mode=Mirror HDMI"
            echo "-h          display this help message"
            echo "-v          display version number"
            echo "-u          set homedir (default=/home/pi)"
            echo "-x          reboot when finished('y' or 'n')"
            echo "-c          pitft version"
            echo "                1: resistive  3.2\", 2.8\", 2.4\"  (320x240)            2: 2.2\" (no touch)  (320x240)"
            echo "                3: capacitive  2.8\"  (320x240)                         4: resistive 3.5\"  (480x320)"
            echo "                5: MINI  1.54\", 1.3\"  (240x240) !! W: KRN UPD !!      6: MINI 1.14\" (135x240) !! W: KRN UPD !! "
            echo "                7: ST7789V 2.0\" (NO TOUCH)  (320x240) !! W: KRN UPD !! 8. UNINSTALL DRIVER!!!!"
            echo "-r          rotation selection   1. 90deg     2. 180deg       3. 270deg    4. 0deg"
            echo "-m          mode selection       1. Console   2. Mirror HDMI"
            exit 0
            shift
            ;;
        -v)
...
echo "SELECTED: $PITFT_SELECT"
echo "ROTATE: $PITFT_ROTATE"
if [[ "$PITFT_SELECT" == "PROMPT" ]]; then
...
# echo "UNINST: a${UNINSTALL}a"
# echo "ROT: a${PITFT_ROTATE}a"
# echo "$PITFT_ROTATE" == "PROMPT"

# if [[ "$UNINSTALL" == "false" ]]; then
  # if [[ "$PITFT_ROTATE" == "PROMPT" ]]; then
   #  echo "ROTATE is PROMPT!"
#   else
 #    echo "ROTATE is NOT PROMPT"
  # fi
   #echo "NOT UNINSTALL"
#else
 #  echo "DO UNINSTALL?"
...
if [[ "$UNINSTALL" == "false" && "$PITFT_ROTATE" == "PROMPT" ]]; then
...
if ! $UNINSTALL;then
    ...
    if [[ "$PITFT_MODE" == "PROMPT" ]]; then
      echo "SELECT DISPLAY MODE"
      selectN "DISPLAY CONSOLE" \
              "MIRROR HDMI"
    fi
    if [[ "$PITFT_MODE" == "1" ]]; then
